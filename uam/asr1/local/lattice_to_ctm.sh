...
if [ $stage -le 0 ]; then
  if [ ! -f $lang/phones/word_boundary.int ] ; then
    $cmd LMWT=$min_lmwt:$max_lmwt $dir/scoring/log/get_ctm.LMWT.log \
      set -e -o pipefail \; \
      mkdir -p $dir/score_LMWT_${word_ins_penalty}/ '&&' \
      ...
      utils/int2sym.pl -f 5 $lang/words.txt  \| tee $dir/score_LMWT_${word_ins_penalty}/$name.utt.ctm \| \
      $resolve_overlaps_cmd \| \
      utils/convert_ctm.pl $data/segments $data/reco2file_and_channel \
      '>' $dir/score_LMWT_${word_ins_penalty}/$name.ctm || exit 1;
  else
    $cmd LMWT=$min_lmwt:$max_lmwt $dir/scoring/log/get_ctm.LMWT.log \
      set -e -o pipefail \; \
      mkdir -p $dir/score_LMWT_${word_ins_penalty}/ '&&' \
