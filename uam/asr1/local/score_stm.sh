...
if [ $stage -le 0 ] ; then
  $cmd LMWT=$min_lmwt:$max_lmwt $dir/scoring/log/score.LMWT.log \
    ...
    cp -f $data/stm $dir/score_LMWT${wip}/stm.unsorted '&&' \
    cp -f $dir/score_LMWT${wip}/${name}.ctm $dir/score_LMWT${wip}/${name}.ctm.unsorted '&&'\
    $SortingProgram sortSTM \<$dir/score_LMWT${wip}/stm.unsorted          \>$dir/score_LMWT${wip}/stm.sorted '&&' \
    $SortingProgram sortCTM \<$dir/score_LMWT${wip}/${name}.ctm.unsorted  \>$dir/score_LMWT${wip}/${name}.ctm.sorted '&&' \
    paste -d ' ' \<\(cut -f 1-5 -d ' ' $dir/score_LMWT${wip}/stm.sorted \) \
                 \<\(cut -f 6- -d ' ' $dir/score_LMWT${wip}/stm.sorted \| uconv -f utf8 -t utf8 -x "$icu_transform" \) \
        \> $dir/score_LMWT${wip}/stm '&&' \
    paste -d ' ' \<\(cut -f 1-4 -d ' ' $dir/score_LMWT${wip}/${name}.ctm.sorted \) \
                 \<\(cut -f 5-  -d ' ' $dir/score_LMWT${wip}/${name}.ctm.sorted \| uconv -f utf8 -t utf8 -x "$icu_transform" \) \
        \> $dir/score_LMWT${wip}/${name}.ctm.sorted2 '&&' \
    utils/fix_ctm.sh $dir/score_LMWT${wip}/stm $dir/score_LMWT${wip}/${name}.ctm.sorted2 '&&' \
    $SortingProgram sortCTM \<$dir/score_LMWT${wip}/${name}.ctm.sorted2  \>$dir/score_LMWT${wip}/${name}.ctm '&&' \
    $ScoringProgram -s -r $dir/score_LMWT${wip}/stm  stm -h $dir/score_LMWT${wip}/${name}.ctm ctm \
