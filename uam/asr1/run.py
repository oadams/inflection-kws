...
def get_args():
    ...
    if not args.custom_kwlist and (args.filt_dtl or args.add_spurious):
...
def prepare_langs_filt_dtl(recog_langs, add_spurious=True, suffix="_filt_dtl"):
    """
    We remove from the lexicon hypotheses that
    ...
    for babel_code in recog_langs:
        ...
        # can be used
        dict_uni_filt = Path(f"data/{babel_code}_test/data/dict_universal{suffix}")
